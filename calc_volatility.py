...
if __name__ == "__main__":
    df_crypto_data = getMarketDataFromCoinmarketcap(numer_of_crypto_currenty=5000, market_cap_min=1000000000,
                                        sort_method="percent_change_24h")
    data = []
    for index, row in df_crypto_data.iterrows():
        ticker = row["Symbol"]
        market_cap = row["Market_cap"]
        print ("calculating growth rate and volatility of crypto {crypto}, with market cap = {market_cap}".format(crypto=ticker, market_cap=market_cap))
        ...
        try:
            ...
            print ("ticker {ticker} is trading on Binance. Currently mean_growth rate is: {growth_rate}; "
                   "variance is : {variance}".format(ticker=binance_ticker, growth_rate=mean_1hr, variance=volatility_1hr))
